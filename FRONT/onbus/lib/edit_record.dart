...
class _EditRecordState extends State<EditRecord> {
  ...
  void _saveEnterprise() async {
    ...
    try {
      if (_isEditing) {
        final ok = await _service.updateEnterprise(widget.enterpriseId!, {
          'name': _nameController.text,
          'cnpj': _cnpjController.text,
          'email': _emailController.text,
          'phone': _phoneController.text,
        });
        if (!ok) throw Exception('Falha ao atualizar');
      }
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(_isEditing ? l10n.companyUpdatedSuccess : l10n.companyRegisteredSuccess),
        ),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
